/* font8x8.c -- embedded 8x8 monochrome VGA bitmap font (U+0000..U+007F).
 *
 * Public Domain. Author: Marcel Sondaar / International Business Machines
 * (public-domain VGA fonts); C array by Daniel Hepper (font8x8, public domain).
 * Used here purely as the TEMPLATE SOURCE for the zoning 1-NN recognizer:
 * a from-scratch reference glyph set, no training data, no dependency.
 *
 * Bit convention: 8 bytes per glyph (one per row, top->bottom); within a
 * byte the LEAST significant bit is the LEFTMOST pixel; a set bit is ink.
 */
#include "font8x8.h"

const unsigned char wubuocr_font8x8[128][8] = {

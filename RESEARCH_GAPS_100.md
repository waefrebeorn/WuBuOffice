...
61. **P1** No images inline (model has SHAPE but no image node; need raster embed). — **CLOSED**: new `WUBUMODEL_IMAGE` kind + `wubumodel_node_set_image`/`wubumodel_node_image` store an embedded RGBA plane (copy, freed on destroy); `wubulayout` treats IMAGE as an object box; the Document view blits the raster into it (alpha-blended, letterboxed). Ctrl+Shift+I inserts a sample checker image. Headless-tested in test_toc (image retained + laid out as a box). Real PNG import would route a decoded wubuimage plane through set_image.
62. **P1** No tables UI (insert/resize/merge cells) though model supports TABLE/CELL. — **CLOSED (insert + render)**: `wubulayout` lays each CELL as a bordered box (new opaque `wubulayout_box` accessor exposes geometry + source node without leaking the internal struct) and stacks rows; the Document view draws cell borders from those boxes; `Ctrl+Shift+T` inserts a 2×2 table. Headless-tested in test_toc (two cell boxes laid out).

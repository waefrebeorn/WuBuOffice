...
58. **P1** No styles system in UI (paragraph/character styles beyond raw prop bag). — **PARTIAL**: the style property bag already supports named styles and is used for headings (DOC-54) and lists (DOC-59, `list=bullet|ol`); UI merely lacks a picker dialog. Left open only as "no style picker UI", not "no styles".

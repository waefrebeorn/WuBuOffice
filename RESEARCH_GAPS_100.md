...
- **DOC-74** (P2) No format-painter. — **CLOSED**: did the model surgery it required — added `wubumodel_style_prop_at(s,i,&name,&value)` to enumerate ALL style props without knowing their names; `src/wubufmtpaint` copies the full prop set from a source run into a brush and applies a *fresh* style per target (no aliasing), so a picked format can be painted onto N nodes. Verified by test_fmtpaint (pick 3 props → apply → dst matches, src not aliased).
- **DOC-75** (P2) No nested tables. — **CLOSED**: `src/wubunesttab` builds TABLE→CELL grids (empty RUN per cell), nests child tables inside any CELL, computes nesting depth, and validates (TABLE children must be CELLs, recursive). Verified by test_nesttab (build 2×2, nest to depth 3, reject non-cell nest, OOB cell NULL).
- **EXP-82** (P2) No PDF forms export. — **CLOSED (full)**: was PARTIAL; `src/wubupdfform` now writes a real, uncompressed PDF 1.4 AcroForm — Catalog/Pages/Page/AcroForm/Helvetica objects + one /Tx (text) or /Btn (checkbox) widget per wubuform field, byte-accurate xref offsets, trailing `%%EOF`. Values escaped; checkbox serializes `/V /Yes|Off`. Verified by test_pdfform (header, both field types, escaped value, byte-accurate startxref, writes /tmp/test_form.pdf).
